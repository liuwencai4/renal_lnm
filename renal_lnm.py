...
#应用标题
st.title('A machine learning-based predictive model for predicting lymph node metastasis in patients with renal cancer')
...
map = {'T1':0,'T2':1,'T3':2,'T4':3,'TX':4,'No':0,'Yes':1,'Well differentiated':0,'Moderately differentiated':1,
       ...
       'C64.9-Kidney':0,'C65.9-Renal pelvis':1,'M0':0,'M1':1}
...
features = ['Primary.Site','Grade','T','M','Tumor.Size','Liver.metastasis']
target = 'Lymph.node.metastasis'
#处理数据不平衡
...
sp = 0.5
#figure
is_t = (XGB.predict_proba(np.array([[Primary_Site,Grade,T,M,Tumor_Size,Liver_metastasis]]))[0][1])> sp
prob = (XGB.predict_proba(np.array([[Primary_Site,Grade,T,M,Tumor_Size,Liver_metastasis]]))[0][1])*1000//1/10
